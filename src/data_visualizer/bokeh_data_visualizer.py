    ...
    def visualize_data(self, x_train, y_train, ideal_functions, x_test, y_test, mapped_data):
        ...
        try:
            for i in range(y_train.shape[1]):
                ...
                p = figure(title=f"Training Data (y{i+1}) and Ideal Function ({ideal_functions.columns[i]})", x_axis_label='x', y_axis_label='y')

                # Plot training data
                p.line(x_train, y_train[:, i], legend_label=f'Training data y{i+1}', line_width=2, color='red')

                # Plot ideal functions
                p.line(x_train, ideal_functions[ideal_functions.columns[i]], legend_label=f'Ideal function {ideal_functions.columns[i]}', line_dash='dashed', line_width=2, color='green')
            ...
            p2.line(mapped_data['x'], mapped_data['ideal_func_val'], legend_label=f'Mapped Ideal Function', line_dash='dashed', line_width=2, color="green")
